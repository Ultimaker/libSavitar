...
void MeshData::fillByXMLNode(pugi::xml_node xml_node)
{
    ...
    this->id = xml_node.attribute("id").as_string();

    // TODO: Add error handling (what to do if there is no vertices object, etc)

    // Add every vertex.
    pugi::xml_node xml_vertices = xml_node.child("mesh").child("vertices");
    ...
    // Add every face.
    pugi::xml_node xml_triangles = xml_node.child("mesh").child("triangles");
